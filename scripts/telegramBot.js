...
let activeChatIds = new Set(['8629428593']); // Store user chat IDs for proactive push reminders
...
function generateScorecardHtml(todayStr, totalCal, totalProt, waterData, walkingLogs, habits) {
  const walkLogged = walkingLogs.some(w => w.date === todayStr);
...
// Handle Incoming Telegram Message or Photo
async function handleMessage(message) {
  const chatId = message.chat.id;
  activeChatIds.add(String(chatId));
...
🚶 <b>Logged Items:</b> ${todayItems.length} meals`;
  ...
  if (incomingText === '💧 +500ml Water') {
    ...
    await sendTelegramMessage(chatId, `💧 <b>Logged +500ml Water!</b>\nTotal today: <b>${(newWater / 1000).toFixed(2)} L</b> / ${(waterData.targetMl / 1000).toFixed(1)} L`, MAIN_KEYBOARD);
    return;
  }
...
4. Add the item to foodLogs[todayStr][category].
...
Respond strictly in this JSON format:
{
  ...
  try {
    ...
    if (aiResponse.ok) {
      ...
      if (parsedAI.updatedFoodLogs) nextPayload.foodLogs = parsedAI.updatedFoodLogs;
      ...
      if (parsedAI.newWeightLog) nextPayload.weightLogs = [...(nextPayload.weightLogs || []), parsedAI.newWeightLog];
      if (parsedAI.waterToAddMl > 0) {
        nextPayload.waterData = { ...waterData, consumedMl: (waterData.consumedMl || 0) + parsedAI.waterToAddMl };
      }
    ...
    await sendTelegramMessage(chatId, `⚠️ Error: ${err.message}`, MAIN_KEYBOARD);
  }
}

// Scheduled Daily Push Reminders (Feature 3: 7:30 AM, 2:00 PM, 6:00 PM, 10:00 PM IST)
let lastSentReminder = '';
function checkScheduledReminders() {
  ...
  // 7:30 AM Morning Weight Reminder
  if (hours === 7 && minutes === 30 && lastSentReminder !== `morning-${timeKey}`) {
    lastSentReminder = `morning-${timeKey}`;
    for (const cid of activeChatIds) {
      sendTelegramMessage(cid, `🌅 <b>Good Morning Nadeer!</b>\n⚖️ Remember to log your morning fasted weight to track progress toward 100 KG!\n\nJust type: <i>"Morning weight 110.5 kg"</i>`, MAIN_KEYBOARD);
    }
  }

  // 2:00 PM Midday Water & Deficit Check
  if (hours === 14 && minutes === 0 && lastSentReminder !== `midday-${timeKey}`) {
    lastSentReminder = `midday-${timeKey}`;
    for (const cid of activeChatIds) {
      sendTelegramMessage(cid, `💧 <b>Hydration & Lunch Check-in!</b>\nStay hydrated this afternoon. Tap <code>💧 +500ml Water</code> or log your lunch!`, MAIN_KEYBOARD);
    }
  }

  // 6:00 PM Evening Walk Reminder
  if (hours === 18 && minutes === 0 && lastSentReminder !== `evening-${timeKey}`) {
    lastSentReminder = `evening-${timeKey}`;
    for (const cid of activeChatIds) {
      sendTelegramMessage(cid, `🚶 <b>Evening Walk Time!</b>\nReady to crush today's 5.0 KM walk? Just show up and get those steps in! 🔥`, MAIN_KEYBOARD);
    }
  }

  // 10:00 PM Night Scorecard & Sleep Reminder
  if (hours === 22 && minutes === 0 && lastSentReminder !== `night-${timeKey}`) {
    lastSentReminder = `night-${timeKey}`;
    for (const cid of activeChatIds) {
      sendTelegramMessage(cid, `🌙 <b>Night Scorecard Check-in!</b>\nTap <code>🎯 Daily Scorecard</code> to see your accomplishments today. Aim for 8 hours of restorative sleep tonight! 😴`, MAIN_KEYBOARD);
    }
  }
}

// Start Long Polling & Scheduler
async function startPolling() {
  console.log('🤖 APEX 100 Telegram Bot (with Vision, Reminders, Scorecard & Dining Advisor) is LIVE...');
